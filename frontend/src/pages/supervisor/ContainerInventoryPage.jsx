      ...
      <div className="rounded-3xl border border-slate-800 bg-slate-900/70 p-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400">Read-only view of every container currently tracked by the terminal.</p>
          </div>
        </div>
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-4">
          ...
          <select className="rounded-xl border border-slate-700 bg-slate-950/70 px-4 py-3 text-slate-100" value={type} onChange={(e) => handleFilterChange(setType)(e.target.value)}>
            <option value="">All ISO types</option>
            <option value="ISO20">ISO20</option>
            <option value="ISO40">ISO40</option>
          </select>
          <select className="rounded-xl border border-slate-700 bg-slate-950/70 px-4 py-3 text-slate-100" value={state} onChange={(e) => handleFilterChange(setState)(e.target.value)}>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            ...
            <tbody>
              ...
              {!loading && !rows.length ? (
                <tr>
                  <td colSpan="7" className="py-6 text-center text-slate-400">No containers match your filters.</td>
                </tr>
              ) : null}
            </tbody>
          </table>
